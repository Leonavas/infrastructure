...
echo -e "Bom dia a todos. \n\nSegue a volumetria do mailing disponibilizado hoje em anexo. \n\nOs arquivos de Mailing estão disponíveis em \santoanastacio\interface\uplift\mailing_offline  \n\nOs arquivos de Mailing de Torpedo Fone para Clientes Novos Sem Recarga está disponível em \santoanastacio\interface\uplift\mailing_offline\nuance\ \n\nEm todos os mailings foram inseridos os NTCs de teste. \n\nPeço que acompanhem diariamente esta volumetria para garantir o alinhamento entre todas as partes envolvidas. \n\nAtenciosamente, \n\nKaren Ximenes \nStefanini Inspiring" |/bin/mail -s "[Mailing Offline] Volumetria ${TODAY}" -a ${TRUNC_DIR}/Volumetria_total_${TODAY}.txt -r kcseno@stefanini.com kcseno@stefanini.com rsmoya@stefanini.com mhcardoso@stefanini.com  vlahr@stefanini.com renata.pelatti@claro.com.br alexandre.rezende@claro.com.br fernanda.santosrodrigues@claro.com.br lilian.stoppa@net.com.br maria.moreno@claro.com.br 
...
###echo -e "Mailing ${TODAY}" |/bin/mail -s "[Mailing Offline] Volumetria ${TODAY}" -a ${TRUNC_DIR}/Volumetria_total_${TODAY}.txt -r kcseno@stefanini.com rsmoya@stefanini.com
...
sftp santoanastacio <<EOF
cd /d/interface/uplift/mailing_offline/nuance
rm *
put *.txt
EOF
