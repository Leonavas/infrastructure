...
${PDI_BIN} -file=${DIR}/pdi/job/run-generic-db-processor.kjb -level=Minimal \
     " -param:acm.job.dir.in="${TMP_DIR}"/out/new" \
     " -param:acm.job.dir.prc="${PRC_DIR} \
     " -param:acm.job.dir.root="${DIR} \
     " -param:acm.job.file.name.regex=new_[0-9]+_[0-9]+\.txt\.gz" \
     " -param:acm.job.param.command=insert"
