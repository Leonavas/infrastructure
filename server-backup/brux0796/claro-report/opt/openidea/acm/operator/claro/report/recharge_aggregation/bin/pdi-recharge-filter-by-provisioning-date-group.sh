...
${PDI_BIN} -file=${DIR}/pdi/job/recharge-filter-by-provisioning-date-group.kjb -level=Basic \
     " -param:acm.job.file.cfg.ndc="${CN_CFG_FILE} \
     " -param:acm.job.recharge_dir="${IN_DIR} > ${DIR}/log/recharge-filter-by-provisioning-date-group.log
