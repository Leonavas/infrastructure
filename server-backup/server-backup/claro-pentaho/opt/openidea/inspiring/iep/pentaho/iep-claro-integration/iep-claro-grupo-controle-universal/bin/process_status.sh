...
input_file_pattern="grupo_controle_universal__*\.txt\.gz"
log_file_pattern="iep-claro-grupo-controle-universal_*"
process_bin="iep-claro-integration/iep-claro-grupo-controle-universal/bin/start_process.sh"
...
if [ -f "${HOME_DIR}/var/pid" ];
then
    pid=$(cat ${HOME_DIR}/var/pid)
fi
