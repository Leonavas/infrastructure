...
JOB_FILE=iep-claro-pacotes-ngp-cancelamento.kjb
...
#   VARIAVEIS
#===============================================================
FILE_DATE=$(date --date='-1 day' +%Y%m%d) #YYYYMMDD
...
function usage()
{
    ...
    echo "Processa arquivo com cancelamento dos pacotes NGP"
    echo "   OPCOES:"
...
function setLogFile()
{
    ...
    LOG_FILE="${HOME_DIR}/log/iep-claro-pacotes-ngp-cancelamento_${now}.log"
...
while getopts :d:l:borh opt; do
    case ${opt} in
        ...
        b)
            ...
            ;;
        r)
            JOB_FILE="iep-claro-pacotes-ngp-cancelamento-reprocessa.kjb"
            ;;
        o)
            setLogFile
            ;;
...
IN_PARAMS="-param:iep.claro.pacotes.ngp.cancelamento.param.date.reference=${FILE_DATE}"
IN_PARAMS="${IN_PARAMS} -param:iep.claro.pacotes.ngp.cancelamento.param.iep.url=${IEP_CLARO_URL}"
