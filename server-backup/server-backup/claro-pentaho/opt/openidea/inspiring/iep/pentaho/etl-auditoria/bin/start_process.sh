...
JOB_FILE=auditoria_transicao_campanha.kjb
...
function usage()
{
    ...
    echo "   -i <DIRETORIO>    Diretorio onde se encontram os arquivos. Se nao informado utiliza do arquivo ${HOME_DIR}/cfg/config.properties"
...
function setLogFile()
{
    ...
    LOG_FILE="${HOME_DIR}/log/etl-auditoria_${FILE_DATE}_${now}.log"
...
while getopts :d:i:l:oh opt; do
    case ${opt} in
        ...
        i)
            ...
            IN_PARAMS="${IN_PARAMS} -param:iep.job.input_dir=${INPUT_DIR}"
            ;;
        l)
            setLogLevel ${OPTARG}
            ;;
...
echo "Diretorio de entrada: $INPUT_DIR"
