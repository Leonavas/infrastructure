...
JOB_FILE=iep-claro-grupo-controle-universal.kjb
...
function usage()
{
    ...
    echo "Processa arquivos de grupo de controle universal"
    echo "   OPCOES:"
...
function setLogFile()
{
    ...
    LOG_FILE="${HOME_DIR}/log/iep-claro-grupo-controle-universal_${now}.log"
...
function get_last_file_date()
{
    ...
    then
        OLD_FILE_DATE=${MOST_RECENT_FILE:25:8}
    fi
}
...
while getopts :d:p:l:borh opt; do
    case ${opt} in
        ...
        b)
            ...
            ;;
        r)
            JOB_FILE="iep-claro-grupo-controle-universal-reprocessa.kjb"
            ;;
        o)
            setLogFile
            ;;
...
IN_PARAMS="-param:iep.claro.grupo.controle.universal.param.date.old=${OLD_FILE_DATE}"
IN_PARAMS="${IN_PARAMS} -param:iep.claro.grupo.controle.universal.param.date.reference=${FILE_DATE}"
IN_PARAMS="${IN_PARAMS} -param:iep.claro.grupo.controle.universal.param.iep.url=${IEP_CLARO_URL}"
