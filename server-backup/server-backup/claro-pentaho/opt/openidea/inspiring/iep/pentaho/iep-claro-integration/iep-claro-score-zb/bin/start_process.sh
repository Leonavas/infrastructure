...
JOB_FILE=iep-claro-score-zb.kjb
...
function usage()
{
    ...
    echo "Processa arquivos de score ZB"
    echo "   OPCOES:"
...
function setLogFile()
{
    ...
    LOG_FILE="${HOME_DIR}/log/iep-claro-score-zb_${now}.log"
...
function get_last_file_date()
{
    ...
    then
        OLD_FILE_DATE=${MOST_RECENT_FILE:9:8}
    fi 
}
...
IN_PARAMS="-param:iep.claro.score.zb.param.date.old=${OLD_FILE_DATE}"
IN_PARAMS="${IN_PARAMS} -param:iep.claro.score.zb.param.date.reference=${FILE_DATE}"
IN_PARAMS="${IN_PARAMS} -param:iep.claro.score.zb.param.iep.url=${IEP_CLARO_URL}"
