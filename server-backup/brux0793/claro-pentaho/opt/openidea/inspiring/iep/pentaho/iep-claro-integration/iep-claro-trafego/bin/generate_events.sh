...
JOB_FILE=iep-claro-trafego-gera-eventos.kjb
...
#   VARIAVEIS
#===============================================================
DATA_FOTO_SALDO=$(date --date='-1 day' +%Y%m%d) #YYYYMMDD
MES_REFERENCIA=$(date --date="$(date +%Y-%m-15) -1 month" +%Y%m)
...
function usage()
{
    echo "Uso: $0 [-boh] [-l <NIVEL_LOG>] [-m <DATA>] [-f <DATA>]"
    echo "Obtem os registros de portin e executa no IEP"
    echo "   OPCOES:"
    echo "   -m  Mes de referencia, no formato AAAAMM. Valor padrao: Mes anterior"
    echo "   -f  Data da foto de saldo. Valor padrao: Dia anterior"
...
function setLogFile()
{
    ...
    LOG_FILE="${HOME_DIR}/log/iep-claro-trafego-gera-eventos_${now}.log"
...
function get_data_foto_saldo()
{
    DATA_FOTO_SALDO=$(date --date="$1" +%Y%m%d)
...
function get_mes_referencia()
{
    MES_REFERENCIA=$(date --date="${1}15" +%Y%m)

    if [ $? -ne 0 ];
    then
        echo "Formato de data invalido: $1"
        echo "Formato esperado: AAAAMM. Exemplo: 201711"
        exit 1
    fi   
}
...
#   READ THE OPTIONS
#===============================================================
while getopts :m:f:l:boh opt; do
    case ${opt} in
        m)
            get_mes_referencia ${OPTARG}
            ;;
        f)
            get_data_foto_saldo ${OPTARG}
            ;;  
        l)
            setLogLevel ${OPTARG}
            ;;
...
echo "Mes de referencia: ${MES_REFERENCIA}"
echo "Data da foto de saldo: ${DATA_FOTO_SALDO}"
echo "Nivel de log: ${LOG_LEVEL}"
echo "Arquivo de log: ${LOG_FILE}"
echo "Executando job ${JOB_FILE}"

IN_PARAMS="-param:iep.claro.trafego.param.date.foto.saldo=${DATA_FOTO_SALDO}"
IN_PARAMS="${IN_PARAMS} -param:iep.claro.trafego.param.date.reference=${MES_REFERENCIA}"
