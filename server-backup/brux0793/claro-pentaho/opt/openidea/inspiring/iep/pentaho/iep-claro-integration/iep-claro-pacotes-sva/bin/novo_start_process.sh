...
JOB_FILE=iep-claro-pacotes-sva.kjb
...
function usage()
{
    ...
    echo "   OPCOES:"
    echo "   -d  Data de processamento, no formato AAAAMMDDHH. Caso nao informado considera o dia e hora atuais"
...
function setLogFile()
{
    ...
    LOG_FILE="${HOME_DIR}/log/iep-claro-pacotes-sva_${now}.log"
...
while getopts :d:l:borh opt; do
    case ${opt} in
        ...
        b)
            ...
            ;;
        r)
            JOB_FILE="iep-claro-pacotes-sva-reprocessa.kjb"
            ;;
        o)
            setLogFile
            ;;
...
IN_PARAMS="-param:iep.claro.pacotes.sva.param.date.reference=${FILE_DATE}"
IN_PARAMS="${IN_PARAMS} -param:iep.claro.pacotes.sva.param.iep.url=${IEP_CLARO_URL}"
